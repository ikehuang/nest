...
function load_page() {
	...
	if(!empty($result)) {
		foreach($result as $r) {
			if($r->post_type == 'page') {
				$sql = "SELECT * FROM wp_postmeta where post_id={$r->ID}";
				$r->meta = $wpdb->get_results($sql, OBJECT);
				$new_result[$r->post_name] = $r;
			}	
		}
...
function get_category_list() {
	global $wpdb;
	$id = 8;
	$sql = "select * from wp_huge_it_catalog_products where catalog_id=8";
